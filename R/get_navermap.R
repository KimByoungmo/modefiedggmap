...
#' ggmap(map)
#' @author Kim BM
#' @description modify with modern naver API
#' @param url Registered Service URL in Naver DEVELOPER Center
#' @description center can alse be inputed address
#' @param secret Naver API Client secret ID
get_navermap <- function(
  ...
  markers, key, url, filename = "ggmapTemp", messaging = FALSE, urlonly = FALSE,
  ...
  geocode_url <- paste0('https://openapi.naver.com/v1/map/geocode','')
  if("center" %in% argsgiven){
    if(!((is.numeric(center))&&length(center) == 2))
      {
      if(is.character(center) && length(center)) {
       if ("secret" %in% argsgiven) {
          if(is.null(secret) | is.na(secret)) {
            stop('Client Secret must be required!', call. = F)
            }
      h <- c('X-Naver-Client-Id' = key, 'X-Naver-Client-Secret' = secret)
      q <- list(query = enc2utf8(center), encoding = 'UTF-8', coord = 'latlng', callback ='', output = 'xml')
      res <- GET(url = geocode_url, add_headers(h), query = q)
      res1 <- xmlRoot(htmlTreeParse(file = res, useInternalNodes = T, getDTD = F, encoding = 'UTF-8'))
      x <- xpathSApply(doc = res1, path = '//point/x', xmlValue)
      y <- xpathSApply(doc = res1, path = '//point/y', xmlValue)
      center <- c(as.numeric(x[1]),as.numeric(y[1]))
  ...
  post_url <- paste(center_url,crs_url ,zoom_url, size_url, baselayer_url, overlayers_url,
                    format_url, markers_url, key_url, url_url, sep = "&")
